...
export default function CandidateDashboard() {
  ...
  const handleDragEnd = useCallback((e: DragEndEvent) => {
    ...
    // Panel → Block: swap chart style
    if (info?.source === "panel" && info.chartType) {
      const targetBlockId = (over.data.current as { blockId?: string } | undefined)?.blockId;
      if (targetBlockId) {
        applyChartType(targetBlockId, info.chartType);
        if (panelBlockId === targetBlockId) {
          // keep panel open, it'll re-read from blocks state
        }
      }
      return;
    }
